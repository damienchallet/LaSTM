...
@keras_export(v1=['keras.layers.LaSTMCell'])
class LaSTMCell(DropoutRNNCellMixin, Layer):
    ...
    def __init__(self,
        ...
        if ops.executing_eagerly_outside_functions():
            ...
            self._enable_caching_device = kwargs.pop('enable_caching_device', False)
        super(LaSTMCell, self).__init__(**kwargs)
        self.units = units    
        ...
        self.alpha_initializer = initializers.constant
    ...
    def get_initial_state(self, inputs=None, batch_size=None, dtype=None):
        return _generate_zero_filled_state_for_cell(self, inputs, batch_size, dtype, factor_size=1)
    ...
    def _compute_carry_and_output(self, x, h_tm1, c1_tm1, c2_tm1, alpha):
        """Computes carry and output using split kernels."""

        x_i, x_f1 , x_f2, x_c1 , x_c2 , x_o =  x

        h_tm1_i, h_tm1_f1, h_tm1_f2, h_tm1_c1, h_tm1_c2, h_tm1_o = h_tm1

        if self.bound_alpha:
            alpha_eff=self.sigmoid(alpha)
        else:
            alpha_eff=alpha
            
        i = self.recurrent_activation(x_i + backend.dot(h_tm1_i, self.recurrent_kernel[:, :self.units]))
        f1 = self.recurrent_activation(x_f1 + backend.dot(h_tm1_f1, self.recurrent_kernel[:, self.units:self.units * 2]))
        f2 = self.recurrent_activation(x_f2 + backend.dot(h_tm1_f2, self.recurrent_kernel[:, 2 * self.units:self.units * 3]))

        c1 = f1 * c1_tm1 + i * self.activation(x_c1 + backend.dot(h_tm1_c1, self.recurrent_kernel[:, self.units * 3:self.units * 4]))
        c2 = f2 * c2_tm1 + i * self.activation(x_c2 + backend.dot(h_tm1_c2, self.recurrent_kernel[:, self.units * 4:self.units * 5]))

        c = alpha_eff * c1 + (1- alpha_eff) * c2

        o = self.recurrent_activation(x_o + backend.dot(h_tm1_o, self.recurrent_kernel[:, self.units * 5: self.units * 6]))

        return  c1 , c2 , c , o


    def _compute_carry_and_output_fused(self, z, c_tm1 , tau1 , tau2):
        """Computes carry and output using fused kernels."""
        # Split the input into 4 tensors for the fused kernel
        z0, z1, z2, z3 = z

        # Compute input gate using the first tensor and apply recurrent activation
        i = self.recurrent_activation(z0)

        # Compute forget gate using the second tensor and apply recurrent activation
        f = self.recurrent_activation(z1)

        # Compute new carry using two time scales
        # First, compute the two smoothing kernels
        # TODO
      #  K1 = lambda x: self.alpha * np.exp(-x/tau1)  # Define smoothing kernel for the first time scale   
      #  K2 = lambda x: (1-self.alpha) * np.exp(-x/tau2)  # Define smoothing kernel for the second time scale
        # Compute c1 and c2 using the two smoothing kernels
        c1, c2 = 0, 0
        for t in range(len(c_tm1)):
            c1 += K1(len(c_tm1)-t) * c_tm1[t]
            c2 += K2(len(c_tm1)-t) * c_tm1[t]

        # Compute the new carry using the two time scales
        c = i * self.activation(z2) + f * (self.alpha * c1 + (1-self.alpha) * c2)

        # Compute output gate using the fourth tensor and apply recurrent activation
    ...
    def call(self, inputs, states, training=True):
        ...
        c12_tm1 = states[1]  # previous carry state
        c1_tm1  = c12_tm1[:,:self.units]
        c2_tm1  = c12_tm1[:,self.units:2*self.units]

        if c2_tm1.shape[1]==0:   # first time
            c2_tm1=c1_tm1

        dp_mask = self.get_dropout_mask_for_cell(inputs, training, count=6)
        rec_dp_mask = self.get_recurrent_dropout_mask_for_cell(h_tm1, training, count=6)


        if self.implementation == 1:
            if 0 < self.dropout < 1.:
                inputs_i = inputs * dp_mask[0]
                inputs_f1 = inputs * dp_mask[1]
                inputs_f2 = inputs * dp_mask[1]
                inputs_c1 = inputs * dp_mask[2]
                inputs_c2= inputs * dp_mask[2]
            ...
            k_i, k_f1 ,k_f2, k_c1, k_c2 ,  k_o = array_ops.split(
                self.kernel, num_or_size_splits=6, axis=1)
                
            x_i = backend.dot(inputs_i, k_i)
            x_f1 = backend.dot(inputs_f1, k_f1)
            x_f2 = backend.dot(inputs_f2, k_f2)
            x_c1 = backend.dot(inputs_c1, k_c1)
            x_c2 = backend.dot(inputs_c2, k_c2)            
            x_o = backend.dot(inputs_o, k_o)
            if self.use_bias:
                b_i, b_f1, b_f2, b_c1, b_c2, b_o = array_ops.split(self.bias, num_or_size_splits=6, axis=0)
                x_i = backend.bias_add(x_i, b_i)
                x_f1 = backend.bias_add(x_f1, b_f1)
                x_f2 = backend.bias_add(x_f2, b_f2)
                x_c1 = backend.bias_add(x_c1, b_c1)
                x_c2 = backend.bias_add(x_c2, b_c2)
                x_o = backend.bias_add(x_o, b_o)

            if 0 < self.recurrent_dropout < 1.:
                h_tm1_i = h_tm1 * rec_dp_mask[0]
                h_tm1_f1 = h_tm1 * rec_dp_mask[1]
                h_tm1_f2 = h_tm1 * rec_dp_mask[1]
                h_tm1_c1 = h_tm1 * rec_dp_mask[2]
                h_tm1_c2 = h_tm1 * rec_dp_mask[2]
         ...
         #   pdb.set_trace()
            ...
            c1, c2, c, o = self._compute_carry_and_output(x, h_tm1, c1_tm1 ,c2_tm1 ,alpha)
            ...
            if 0. < self.dropout < 1.:
                ...
                if self.use_bias:
                    ...
                    tau1 = 0.2
                    tau2 = 0.8
                    c, o = self._compute_carry_and_output_fused(z, c1_tm1,tau1 , tau2)
        h = o * self.activation(c)

#        h12 = tf.concat([h,h],1)
        c12 = tf.concat([c1,c2],1)
        return h, [h, c12]
    ...
    def get_config(self):
        config = {
            ...
            'alpha':
                self.alpha }
        config.update(_config_for_enable_caching_device(self))
        base_config = super(LaSTMCell, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
@keras_export('keras.experimental.PeepholeLaSTMCell')
class PeepholeLaSTMCell(LaSTMCell):
    ...
    def __init__(self,
        ...
        warnings.warn('`tf.keras.experimental.PeepholeLSTMCell` is deprecated '
                      ...
                      'instead.')
        super(PeepholeLaSTMCell, self).__init__(
            units=units,
            activation=activation,
    ...
    def build(self, input_shape):
        super(PeepholeLaSTMCell, self).build(input_shape)
    ...
    def _compute_carry_and_output(self, x, h_tm1, c1_tm1 , c2_tm1):
        x_i, x_f1, x_f2 , x_c1 , x_c2, x_o = x
        h_tm1_i, h_tm1_f1, h_tm1_f2 , h_tm1_c1 , h_tm1_c2, h_tm1_o = h_tm1
        i = self.recurrent_activation(
            x_i + backend.dot(h_tm1_i, self.recurrent_kernel[:, :self.units]) +
            self.input_gate_peephole_weights * c1_tm1)
        f1 = self.recurrent_activation(x_f1 + backend.dot(
            h_tm1_f1, self.recurrent_kernel[:, self.units:self.units * 2]) +
                                      self.forget_gate_peephole_weights * c1_tm1)
        f2 = self.recurrent_activation(x_f2 + backend.dot(
            h_tm1_f1, self.recurrent_kernel[:, self.units:self.units * 2]) +
                                      self.forget_gate_peephole_weights * c2_tm1)

        c1 = f1 * c1_tm1 + i * self.activation(x_c1 + backend.dot(
            h_tm1_c1, self.recurrent_kernel[:, self.units * 2:self.units * 3]))
        c2 = f2* c2_tm1 + i * self.activation(x_c2 + backend.dot(
            h_tm1_c2, self.recurrent_kernel[:, self.units * 2:self.units * 3]))
        c = c1 * alpha + c2 * (1-alpha)
        o = self.recurrent_activation(
            x_o + backend.dot(h_tm1_o, self.recurrent_kernel[:, self.units * 3:]) +
            self.output_gate_peephole_weights * c)

        print("peephole LaSTM, what am I doing here ?")
        return c, o
...
@keras_export(v1=['keras.layers.LaSTM'])
class LaSTM(RNN):
    ...
    def get_config(self):
        ...
        config.update(_config_for_enable_caching_device(self.cell))
        base_config = super(LaSTM, self).get_config()
        del base_config['cell']
        return dict(list(base_config.items()) + list(config.items()))
...
def _generate_zero_filled_state_for_cell(cell, inputs, batch_size, dtype, factor_size=1):
    if inputs is not None:
        batch_size = array_ops.shape(inputs)[0]
        dtype = inputs.dtype

    if factor_size!=1:
        cell.state_size=[factor_size*x for x in cell.state_size]
